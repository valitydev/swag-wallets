%% -*- mode: erlang -*-
-module(swag_client_payres_common_validator).
...
-type result() ::
    ok | {ok, Prepared :: swag_client_payres:value()} | error | {error, Message :: term()}.
...
-type value()    :: swag_client_payres:value().
...
validate({type, 'boolean'}, Value) ->
    case swag_client_payres_utils:to_lower(Value) of
        <<"true">> ->  {ok, true};
        <<"false">> -> {ok, false};
        _ -> error
    end;
