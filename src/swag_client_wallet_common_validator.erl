%% -*- mode: erlang -*-
-module(swag_client_wallet_common_validator).
...
-type result() ::
    ok | {ok, Prepared :: swag_client_wallet:value()} | error | {error, Message :: term()}.
...
-type value()    :: swag_client_wallet:value().
...
validate({type, 'boolean'}, Value) ->
    case swag_client_wallet_utils:to_lower(Value) of
        <<"true">> ->  {ok, true};
        <<"false">> -> {ok, false};
        _ -> error
    end;
