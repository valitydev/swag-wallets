...
validate_format(Value, {?FORMAT, Type}, State) when
    ...
    Type =:= <<"binary">>
->
    case swag_client_wallet_common_validator:validate({type, erlang:binary_to_atom(Type, utf8)}, Value) of
        error ->
...
validate_format(Value, {?FORMAT, Format}, State) when
    ...
    Format =:= <<"uri">>
->
    case swag_client_wallet_common_validator:validate({format, erlang:binary_to_atom(Format, utf8)}, Value) of
        error ->
            jesse_error:handle_data_invalid(wrong_format, Value, State);
        _ ->
            State
