-module(swag_server_payres_custom_validator).
-export([validate_param/4]).
-export([validate_schema/3]).

-type param_rule()   :: swag_server_payres_param_validator:param_rule().
-type schema_rule()  :: swag_server_payres_schema_validator:schema_rule().
-type value()        :: swag_server_payres:value().

-type validation_opts() :: swag_server_payres_validation:validation_opts().
-type param_context()      :: swag_server_payres_param_validator:context().
-type schema_context()     :: swag_server_payres_schema_validator:context().
