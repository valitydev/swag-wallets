%% -*- mode: erlang -*-
-module(swag_server_payres_handler_api).
...
-type request_spec() :: [{
    swag_server_payres:param_name(),
    #{source := param_source(), rules := [swag_server_payres_validation:rule()]}
}].

-type response_spec() :: swag_server_payres_validation:response_spec().
...
-spec authorize_api_key(
    LogicHandler :: module(),
    OperationID  :: swag_server_payres:operation_id(),
    From         :: header | qs_val,
    KeyParam     :: iodata() | atom(),
    Req          :: cowboy_req:req(),
    Context      :: swag_server_payres:request_context()
)->
    {true, swag_server_payres:auth_context(), cowboy_req:req()} |
...
authorize_api_key(LogicHandler, OperationID, From, KeyParam, Req0, Context) ->
    ...
    case ApiKey of
        ...
        _ ->
            Result = swag_server_payres_logic_handler:authorize_api_key(
                LogicHandler,
                OperationID,
                ApiKey,
                Context
...
-spec determine_peer(Req :: cowboy_req:req()) ->
        {ok, Peer :: swag_server_payres:client_peer()} | {error, Reason :: malformed | einval}.
...
-spec populate_request(
    ...
    Spec :: request_spec(),
    Req :: cowboy_req:req(),
    ValidationOpts :: swag_server_payres_validation:validation_opts()
) ->
    {ok,    Populated :: swag_server_payres:object(),       Req :: cowboy_req:req()} |
    {error, Message   :: swag_server_payres:error_reason(), Req :: cowboy_req:req()}.
...
-spec validate_response(
    OperationID :: swag_server_payres:operation_id(),
    Spec     :: response_spec(),
    RespBody :: swag_server_payres:object() | [swag_server_payres:object()] | undefined,
    ValidationOpts :: swag_server_payres_validation:validation_opts()
) ->
    ok | no_return().

validate_response(OperationID, Spec, RespBody, ValidationOpts) ->
    case swag_server_payres_validation:validate_response(OperationID, Spec, RespBody, ValidationOpts) of
        ok ->
            ok;
        {error, Error} ->
...
-spec encode_response(Resp :: swag_server_payres:response()) ->
    Encoded :: response().
...
-spec process_response(
    Status      :: ok | error,
    Result      :: response() | swag_server_payres:error_reason(),
    Req         :: cowboy_req:req(),
    OperationID :: swag_server_payres:operation_id()
) ->
    Req :: cowboy_req:req().
...
populate_request_param(LogicHandler, OperationID, {Name, #{rules := Rules, source := Source}}, Req0, ValidationOpts) ->
    case get_value(Source, Name, Req0) of
        {ok, Value, Req} ->
            case swag_server_payres_validation:prepare_request_param(OperationID, Rules, Name, Value, ValidationOpts) of
                {ok, Result} ->
                    {ok, Name, Result, Req};
                {error, Error}  ->
                    {error, swag_server_payres_logic_handler:map_error(LogicHandler, {validation_error, Error}), Req}
            end;
        {error, Message, Req} ->
            Error = #{
                type => wrong_body,
...
-spec get_value(
    Source :: param_source(),
    Name   :: swag_server_payres:param_name(),
    Req    :: cowboy_req:req()
) ->
    {ok,    Value   :: swag_server_payres:value(),        Req :: cowboy_req:req()} |
    {error, Message :: swag_server_payres:error_reason(), Req :: cowboy_req:req()}.

get_value(body, _Name, Req0) ->
    case swag_server_payres_utils:get_body(Req0) of
        {ok, Body, Req} ->
...
get_value(header, Name, Req) ->
    ...
    {ok, maps:get(swag_server_payres_utils:to_header(Name), Headers, undefined), Req};
get_value(binding, Name, Req) ->
    Bindings = cowboy_req:bindings(Req),
    {ok, maps:get(swag_server_payres_utils:to_binding(Name), Bindings, undefined), Req}.

-spec decode_body(Body :: binary()) ->
    {ok,    Decoded :: swag_server_payres:object() | undefined} |
    {error, Message :: swag_server_payres:error_reason()}.
decode_body(<<>>) ->
    {ok, undefined};
decode_body(Body) ->
    try
