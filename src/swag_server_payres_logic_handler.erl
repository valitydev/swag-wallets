-module(swag_server_payres_logic_handler).
...
-type auth_context()    :: swag_server_payres:auth_context().
-type object()          :: swag_server_payres:object().
-type request_context() :: swag_server_payres:request_context().
-type handler_opts(T)   :: swag_server_payres:handler_opts(T).
-type logic_handler(T)  :: swag_server_payres:logic_handler(T).
-type response()        :: swag_server_payres:response().

-type validation_error() :: swag_server_payres_validation:error().
-type error_type()      :: validation_error.
-type error_message()   :: swag_server_payres:error_reason().
