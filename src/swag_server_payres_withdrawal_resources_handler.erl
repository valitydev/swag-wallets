...
-record(state, {
    operation_id  :: swag_server_payres:operation_id(),
    logic_handler :: module(),
    swagger_handler_opts :: swag_server_payres_router:swagger_handler_opts(),
...
-spec init(Req :: cowboy_req:req(), Opts :: swag_server_payres_router:init_opts()) ->
    {cowboy_rest, Req :: cowboy_req:req(), State :: state()}.

init(Req, {_Operations, LogicHandler, SwaggerHandlerOpts} = InitOpts) ->
    OperationID    = swag_server_payres_utils:get_operation_id(Req, InitOpts),
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'GetBankCard' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
    From = header,
    Result = swag_server_payres_handler_api:authorize_api_key(
        LogicHandler,
        OperationID,
        From,
        'Authorization',
...
is_authorized(
    Req0,
    State = #state{
        operation_id  = 'StoreBankCard' = OperationID,
        logic_handler = LogicHandler,
        context       = Context
    }
) ->
...
malformed_request(Req, State = #state{context = Context}) ->
    PeerResult = swag_server_payres_handler_api:determine_peer(Req),
    case PeerResult of
        {ok, Peer} ->
...
populate_request(LogicHandler, OperationID, Req, ValidationOpts) ->
    ...
    swag_server_payres_handler_api:populate_request(LogicHandler, OperationID, Spec, Req, ValidationOpts).

handle_request(LogicHandler, OperationID, Populated, Context) ->
    swag_server_payres_logic_handler:handle_request(LogicHandler, OperationID, Populated, Context).
...
validate_response(ok, {Code, _Headers, Body}, OperationID, ValidationOpts) ->
    ...
    swag_server_payres_handler_api:validate_response(OperationID, Spec, Body, ValidationOpts).

encode_response(Resp) ->
    swag_server_payres_handler_api:encode_response(Resp).

process_response(Status, Result, Req0, State = #state{operation_id = OperationID}) ->
    Req = swag_server_payres_handler_api:process_response(Status, Result, Req0, OperationID),
    {stop, Req, State}.

validate_headers(_, Req) ->
    {true, Req}.

-spec get_request_spec(OperationID :: swag_server_payres:operation_id()) ->
    Spec :: swag_server_payres_handler_api:request_spec() | no_return().


get_request_spec('GetBankCard') ->
    [
        {'X-Request-ID', #{
            source => header,
...
get_request_spec('StoreBankCard') ->
    [
        {'X-Request-ID', #{
            source => header,
            rules  => [{type, 'binary'}, {max_length, 32}, {min_length, 1}, true
...
-spec get_response_spec(OperationID :: swag_server_payres:operation_id(), Code :: cowboy:http_status()) ->
    Spec :: swag_server_payres_handler_api:response_spec() | no_return().


get_response_spec('GetBankCard', 200) ->
...
get_response_spec('StoreBankCard', 201) ->
